...
export ANSIBLE_VERSION="${ANSIBLE_VERSION:-2.8.8}"
...
export ANSIBLE_PYTHON_INTERPRETER="${ANSIBLE_PYTHON_INTERPRETER:-/usr/bin/python}"
OPTS+=('ANSIBLE_PYTHON_INTERPRETER')
...
if [[ ! -e "${ANSIBLE_EMBED_HOME}/bin/ansible" ]]; then
  if [ ${ID} = "ubuntu" ]; then
    case ${VERSION_ID} in
      20.04)
        ...
        apt update
        apt install -y python2 python2-dev virtualenv
        export ANSIBLE_PYTHON_INTERPRETER=${ANSIBLE_EMBED_HOME}/bin/python
        ;;
      *)
        apt-get update
  ...
  if [[ -f "/usr/bin/python2" ]]; then
    ...
    ${ANSIBLE_EMBED_HOME}/bin/pip install --upgrade setuptools\<45
